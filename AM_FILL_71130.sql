...
/*
  Exception messages:
    #Workbook!... 
    #Sheet!...
    #REF!... unknown sheet/named range, vertical merged cells with insert mode,
             merged cells alignment
*/
...
--debug(p_range_name||'|'||t_loc.sht_nr||'|'||t_loc.row_nr||'|'||t_loc.col_nr||'|'||t_loc.col_off||'|'||t_loc.row_off); 
  ...
  then 
    ...
    then 
      raise_application_error(c_errcode,'#REF!: '||p_range_name); 
    else 
      t_loc.sht_nr := null; 
    end if; 
  end if; 
...
--
Function getSheetXMLPath(p_sheetName varchar2, p_xml xmlType := null) return varchar2
is
  t_xml xmlType;
  t_rid varchar2(20);
...
--
Function getSheetXMLPath(p_sht_nr pls_integer) return varchar2
is
  t_range tp_one_range;
begin
  t_range := context.ran_ind(p_sht_nr);
  return getSheetXMLPath(t_range.range_name);
end;
-- 
Procedure loadWorkBook(p_xlsx BLOB) 
is 
  t_val varchar2(4000); 
...
begin
  ...
  context.current_sht := dbms_xslprocessor.valueof( t_nd, '/workbook/bookViews/workbookView/@activeTab', c_sheetns ) + 1;
-- Google Docs!!!
...
-- load sheets
  ...
  loop 
--    t_ind := dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ), '@sheetId', c_rns ); 
--    t_ind := substr(dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ), '@r:id', c_rns ),4); 
    t_ind := i + 1;
--debug(t_ind); 
-- load sheet merged cells
    add_range(dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ), '@name' ), '',t_ind); 
    t_nd1 := blob2node(as_zip.get_file( p_xlsx, getSheetXMLPath( t_ind ) )); 
    t_nl1 := dbms_xslprocessor.selectnodes(t_nd1, '/worksheet/mergeCells/mergeCell',c_sheetns); 
    for j in 0..dbms_xmldom.getlength( t_nl1 )-1 
    loop 
      t_loc:=addr2loc(dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl1, j ),'@ref',c_sheetns)); 
...
begin 
  clear_styles; 
  t_nd := blob2node( as_zip.get_file(p_xlsx, getSheetXMLPath(p_sht_nr) ) ); 
...
begin 
  ...
  loadWorkBook(context.template); 
end; 
-- 
Procedure replaceXMLnode(p_xml in out xmlType, p_xpath varchar2, p_newnode CLOB) 
as 
begin 
...
Function addRelsType(p_target varchar2, p_type varchar2, p_ctype varchar2) return number 
is 
  t_xml xmltype; 
  t_rid number; 
...
--              end if; 
  ...
  t_xml:=get1xml(context.template, getSheetXMLPath(p_sheet_name)); 
  replaceXMLnode(t_xml, '/worksheet/sheetData', t_clob); 
  replaceXMLnode(t_xml, '/worksheet/mergeCells', t_merges);
  ...
  replace1xml(context.template 
    , getSheetXMLPath(nvl(p_new_sheet_name, p_sheet_name)), t_xml); 
...
begin 
  ...
  if context.str_cnt_of > 0 then 
    ...
    s := addRelsType('sharedStrings.xml' 
...
-- upper???
  begin
    t_sht_nr := context.ranges(upper(p_sheet_name)); -- source sheet or named range exists?
    t_pos := context.fields(t_sht_nr).first;  -- source sheet is filled?
  exception
    when others then raise_application_error(c_errcode, '#Sheet! "'||p_sheet_name||'" not filled sheet');
  end;
-- destination sheet or named range exists?
  if context.ranges.exists(upper(p_newsheet_name)) 
     or getSheetXMLPath(p_newsheet_name) is not null then
    raise_application_error(c_errcode, '#Sheet! "'||p_sheet_name||'" name already exists');
  end if;
...
-- get new rId 
  t_rid := addRelsType('worksheets/sheet'||t_nid||'.xml' 
      ,'http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet' 
      ,'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml'); 
-- insert new sheet record
  t_str := '<sheet name="'||p_newsheet_name 
