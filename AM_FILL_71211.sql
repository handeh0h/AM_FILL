...
begin 
  ...
  if t_c is null then return t_str; end if; -- row style only 
  while t_c is not null loop 
    ...
    if t_st.v is not null or t_st.s is not null then 
      t_str := t_str || '<c r="' || cell2alfan(p_row, t_c) || '"' 
        ...
        || case when t_st.v is null then '/>' else '><v>' || t_st.v || '</v></c>' end; 
    end if; 
...
begin 
  ...
  loop 
    ...
    if dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, j ), 'f' ) is not null 
    then continue; end if; -- function 
    t_t := dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, j ), '@t' );    
    t_v := dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, j ), 'v' );    
    if t_t is null or regexp_instr(t_t,'^(n|s)$') > 0 then   
...
begin 
  if p_fld.value is null then 
    p_rstyle.cells(p_c).v := null; 
    p_rstyle.cells(p_c).t := null; 
  else 
...
--debug(generate_row(t_rsi,t_row)); 
      end loop; 
    else 
      t_row := t_row + 1; 
