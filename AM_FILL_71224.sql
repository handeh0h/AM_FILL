...
begin   
  ...
  if dbms_xmldom.isNull(t_nd) then return; end if;
  t_nl := dbms_xslprocessor.selectnodes(t_nd, '/worksheet/mergeCells/mergeCell',c_sheetns);
  for i in 0 .. dbms_xmldom.getlength( t_nl )-1 
  loop      
    t_loc:=addr2loc(dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ),'@ref',c_sheetns));   
...
-- load sheets     
  ...
  for i in 0 .. dbms_xmldom.getlength( t_nl ) - 1      
  loop      
    add_range(dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ), '@name' ), null);      
  end loop;     
-- load named ranges     
...
--   
Procedure style_field(p_rstyle in out nocopy tp_row_style, p_c pls_integer, p_fld tp_one_field)   
is   
begin   
...
--   
Function generate_sheet_data(p_sht_nr pls_integer) return CLOB   
is   
  t_nd dbms_xmldom.domnode;      
  t_nl dbms_xmldom.domnodelist;      
  t_sheetd CLOB;      
  ...
  t_rst0 tp_row_style; -- source row cells styles   
  t_rsti tp_row_style; -- current row cells styles   
  t_rste tp_row_style; -- empty style   
  t_fld tp_one_field;      
begin     
  t_s := p_sht_nr;    
  if not context.fields.exists(t_s) then return ''; end if;  -- sheet not filled     
  dbms_lob.createtemporary(t_sheetd, false);      
  t_nd := blob2node( as_zip.get_file(context.template, get_sheet_path(t_s) ) );   
...
--debug(t_rm,dbms_xslprocessor.valueof( dbms_xmldom.item( t_nl, i ), '@r' ));   
  ...
  loop   
    ...
    t_rst0 := get_row_styles(dbms_xmldom.item( t_nl, 0));   
    t_rste.style := t_rst0.style;   
    if context.fields(t_s).exists(t_r) then   
      for t_i in 0 .. context.fields(t_s)(t_r).count - 1   
      loop   
        if t_i = 0 then t_rsti := t_rst0; else t_rsti := t_rste; end if;   
--debug(t_s,t_r,t_i);   
...
--debug(t_s,t_r,t_i,t_c);   
            ...
            loop   
              if t_rst0.cells.exists(i) then t_rsti.cells(i) := t_rst0.cells(i); end if;   
            end loop;   
          end if;   
--debug(t_s,t_r,t_i,t_c);   
          style_field(t_rsti, t_c, context.fields(t_s)(t_r)(t_i)(t_c));   
          t_c := context.fields(t_s)(t_r)(t_i).next(t_c);   
        end loop;   
        t_row := t_row + 1;   
        dbms_lob.append(t_sheetd, generate_row(t_rsti, t_row));   
--debug(xmlType(generate_row(t_rsi,t_row)).getclobval(1,1));   
      end loop;   
    else   
      t_row := t_row + 1;   
      dbms_lob.append(t_sheetd, generate_row(t_rst0, t_row)); 
...
--   
Function loc2merge_node(p_loc tp_loc) return varchar2   
is   
begin   
  return '<mergeCell ref="'||cell2alfan(p_loc.row_nr, p_loc.col_nr)   
    ||':'||cell2alfan(p_loc.row_nr + p_loc.row_off, p_loc.col_nr + p_loc.col_off)   
    ||'"/>';      
end;   
--   
Function generate_sheet_merges(p_sht_nr pls_integer) return CLOB   
is   
  t_merges CLOB := EMPTY_CLOB;   
  t_merges_cnt pls_integer := 0;   
  t_mloc tp_loc;      -- merge loc
  t_mc pls_integer;   -- merge column
  t_icnt pls_integer; -- rows inserted (row)
  t_maxr pls_integer; -- max row_nr fields/merges
  t_off pls_integer := 0; -- rows inserted (total)
  t_s pls_integer := p_sht_nr;
begin   
  dbms_lob.createtemporary(t_merges, false);   
  t_maxr := greatest(context.fields(t_s).last, context.merges(t_s).last);
  if t_maxr is null then return t_merges; end if;
  t_merges := '<mergeCells '||c_sheetns||' count="xxx">'; --init CLOB for append   
  t_off := 0;
  for r in 1..t_maxr loop
    t_icnt := 0;
    if context.fields(t_s).exists(r) then
      t_icnt := context.fields(t_s)(r).count - 1;
    end if;
    if context.merges(t_s).exists(r) then
      t_mc := context.merges(t_s)(r).first;
      while t_mc is not null loop
        t_mloc := merge2loc(t_s, r, t_mc);
--debug(t_mloc.row_off,t_mloc.col_off);
        for i in 0..t_icnt loop
          t_mloc.row_nr := r + t_off + i;
          dbms_lob.append(t_merges, loc2merge_node(t_mloc));   
          t_merges_cnt:= t_merges_cnt + 1;
          exit when not field_exists(t_s, r, i+1, t_mc);
        end loop;
        t_mc := context.merges(t_s)(r).next(t_mc);
      end loop;
      t_off := t_off + t_icnt;
    end if;
  end loop;
...
--
Procedure extend_sheet_ranges(p_sht_nr pls_integer)
is
  t_roff pls_integer;
  t_r pls_integer;
  t_c pls_integer;
...
-- select sheet ranges
  ...
  for i in 1..context.ran_ind.count loop
    if not context.ran_ind(i).isSheet and context.ran_ind(i).sht_nr = p_sht_nr then
...
-- shift and extend
  for i in 1..t_rans.count loop
    t_1ran := t_rans(i);
    t_r := context.fields(p_sht_nr).first;
    while t_r is not null and t_r <= (t_rans(i).row_nr + t_rans(i).row_off) loop
      if t_r < t_rans(i).row_nr then
        t_1ran.row_nr := t_1ran.row_nr + context.fields(p_sht_nr)(t_r).count - 1; --shift 
      end if;
      if t_r between t_rans(i).row_nr and (t_rans(i).row_nr + t_rans(i).row_off) then
        t_roff := 0;
        t_c := context.fields(p_sht_nr)(t_r)(0).first;
        while t_c is not null loop
          if t_c between t_1ran.col_nr and (t_1ran.col_nr + t_1ran.col_off) then
            t_roff := greatest(t_roff, ins_count(p_sht_nr, t_r, t_c) - 1);
--debug(t_1ran.row_nr,t_r,t_c);
          end if;
          t_c := context.fields(p_sht_nr)(t_r)(0).next(t_c);
        end loop;
        t_1ran.row_off := t_1ran.row_off + t_roff; --extend
      end if;
      t_r := context.fields(p_sht_nr).next(t_r);
    end loop;
    t_rans(i) := t_1ran;
...
--
Procedure hide_sheet
( p_xml in out nocopy XMLType -- xl/workbook.xml
, p_sheet_name varchar2 -- unquoted shet name
, p_hide boolean := true
)
is
  t_state varchar2(10) := case when p_hide then 'hide' else 'visible' end;
begin
  begin     
    select insertChildXML(p_xml      
      , '/workbook/sheets/sheet[@name="'||p_sheet_name||'"]'     
      , '@state'      
      , t_state      
      , c_sheetns )      
    into p_xml from dual;      
  exception -- assume attribute @state exists     
    when others then     
      select updateXML(p_xml      
        , '/workbook/sheets/sheet[@name="'||p_sheet_name||'"]/@state'     
        , t_state      
        , c_sheetns )      
    into p_xml from dual;      
  end;     
...
as    
  t_xname varchar2(200) := 'xl/workbook.xml';    
  t_xml xmltype;      
...
-- hide/visible source sheet (without saving data)
  if p_newsheet_name is null then
    t_xml := get1xml( context.template, t_xname );
    hide_sheet(t_xml, p_sheet_name, instr(p_options,'h') > 0);
    replace1xml(context.template, t_xname, t_xml);      
    return;
  end if;
...
-- hide source sheet
  if instr(p_options,'h') > 0 then
    hide_sheet(t_xml, t_sheet_name, true);
  end if;     
...
begin    
  ...
  t_loc := loc2cell(addr2loc(p_address,false));    
  ...
  if instr(p_options,'R') > 0 then -- move outside merge in row   
    t_loc.col_nr := t_mloc.col_nr + t_mloc.col_off + 1;    
  end if;    
  if instr(p_options,'B') > 0 then -- move outside merge in column
    t_loc.row_nr := t_mloc.row_nr + t_mloc.row_off + 1;    
  end if; 
  if (instr(p_options,'B') > 0 or instr(p_options,'R') > 0) then
    t_mloc := get_merge(t_loc);
  end if;
  if instr(p_options,'l') > 0 then -- align to left cell in merge    
    t_loc.col_nr := t_mloc.col_nr;    
  end if;    
  if instr(p_options,'t') > 0 then -- align to top cell in merge    
